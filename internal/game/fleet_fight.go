...
//
// Returns any error..
func (d *defender) round(a *attacker, data Instance) error {
	...
	err := d.addShipsDebris(au.ships, data.Ships)
	if err != nil {
		return err
	}
	...
	for _, def := range du.defenses {
		if def.hull <= 0.0 {
			...
			for res, amount := range s.Cost.InitCosts {
				...
				if ok {
					ex.Amount += d.defensesToRuins * float32(amount)
				} else {
	...
	err = d.addShipsDebris(du.indigenous, data.Ships)
	if err != nil {
		return err
	}

	err = d.addShipsDebris(du.reinforcements, data.Ships)
	if err != nil {
		return err
	}
...
//
// Returns any error.
func (d *defender) addShipsDebris(units []unit, ships *model.ShipsModule) error {
