...
func (sa *shipAdapter) Route() string {
	return "ships"
}

// ParseFilters :
// Implementation of the method to get the filters from the route
// variables. This will allow to fetch ships through a precise ID.
//
...
func (sa *shipAdapter) ParseFilters(vars handlers.RouteVars) []handlers.Filter {
	...
	// provide a filter on the ships's identifier. More precisely
	// the route can define something like:
	// `/ships/ship-id` which we will interpret as a filter on the
	// ship's identifier.
...
// Data :
// Implementation of the method to get the data related to ships from
// the internal DB. We will use the internal DB proxy to get the info
// while still applying the filters.
//
...
func (sa *shipAdapter) Data(filters []handlers.Filter) (interface{}, error) {
	...
	return sa.proxy.Ships(dbFilters)
}

// listShips :
// Creates a handler allowing to serve requests on the ships by
