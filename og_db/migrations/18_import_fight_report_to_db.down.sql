...
DROP FUNCTION fleet_fight_report_footer(player_id uuid, pillage json, debris json, rebuilt integer, moment timestamp with time zone, report_id uuid, pOffset integer);

-- Drop the function handling the generation of the fight status.
DROP FUNCTION fleet_fight_report_status(player_id uuid, outcome text, moment timestamp with time zone, report_id uuid, pOffset integer);

-- Drop the function handling the generation of the defender participant of a fight.
DROP FUNCTION fleet_fight_report_indigenous_participant(player_id uuid, planet_id uuid, kind text, ships_remains json, def_remains json, moment timestamp with time zone, report_id uuid, pOffset integer);
